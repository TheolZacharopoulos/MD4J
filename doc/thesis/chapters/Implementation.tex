...
% methods order its not the same as they are defined in the source code, so they can not be used in the initialization of the values.
% no contractor with init values
% alternatives:
% Alphabetical order. (its gonna be used only as an alternative)
...
% It’s important the data managers to inherit from a base one, thats the modular aspect (BasicFactory).
