...
Finally, by using the \texttt{factory of T} the programmer can create instances of \texttt{T} (t) based on the specifications described by the data manager.

\begin{figure}[H]
\centering
\[Schema \ Loading : Factory_{(schema)} \times \overline{Class\langle ? \rangle}^{T} \Rightarrow Schema_{(T)}\]
\[\dots (Data \ Managers) : Schema_{(T)} \times Class\langle Factory_{(T)}\rangle \Rightarrow Factory_{(T)}\]
\[ Instance \ Creation : Factory_{(T)} \Rightarrow t\]
...
The \texttt{M} interface defines a single method \texttt{schemaKlass}, which refers to the \texttt{Klass} that an object is described with.
This \texttt{Klass} is used by the interpreter in order to get the structural description of a managed object. 
...
\paragraph{Two proxies process} \mbox{}\\
