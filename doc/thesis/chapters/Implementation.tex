...
In general, in managed data, the programmer has to simply define schemas and factories of data instances, namely managed objects, described by these schemas.
In order to do that, our framework provides the programmer with a loading process, which parses the schemas defined and uses data managers to manipulate its instances.
The data managers return a new factory instance that creates managed objects.
Depending on the data manager, the managed objects may have different properties, such as \ac{ccc}.
% TODO: Keep this?
...
\[Manage : Schema_{T} \times Class\langle Factory_{T}\rangle \rightarrow Factory_{T} \nonumber \]
...
First, the schema loader gets a \texttt{factory} of schema schemas
\footnote{
...
and a set of \texttt{classes} that describe a schema of type \texttt{T}.
With this information the schema loader parses the arguments and returns a schema of \texttt{T}.
...
Finally, by using the \texttt{factory of T} the programmer can create instances of \texttt{T} based on the specifications described by the data manager.
...
\texttt{SchemaLoader}'s \texttt{load} static method takes as input a set of interfaces, which are the schema definitions, and a \texttt{SchemaFactory}, which is an \texttt{IFactory} that creates instances of \texttt{schemaSchema} provided by the framework, Section \ref{Self-Describing Schemas} explains it further.
...
SchemaFactory schemaFactory = 
	basicFactory.factory(SchemaFactory.class, schemaSchema);
...
A data manager is initialized with a \texttt{Schema} instance and an \texttt{IFactory} class.
...
Finally, Line \ref{line:basic_data_manager_definition_instance} illustrates how the managed object instances with those specifications can be built using the \texttt{IFactory}.
...
In order to create a stack of data managers that combine behavior and specifications, we use inheritance.
