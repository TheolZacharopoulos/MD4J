...
% After a fan-in analysis of JHotDraw \cite{marin2004identifying}, Marin et al. identified the  ``Undo'' concern in JHotDraw and he presents an approach to its aspect-oriented refactoring \cite{marin2004refactoring}. 
% They used the \textit{(un)pluggability} modularity property as an estimate of its refactoring cost.
% The authors propose the refactoring of the ``Undo'' concern in JHotDraw using AspectJ with the implementation of AJHotDraw. 
...
% In this thesis we investigated both the \texttt{FigureSelectionListener} and the \texttt{Undo} concerns by refactor them in a new version of the JHotDraw and compare them to their \ac{aop} counterpart.
